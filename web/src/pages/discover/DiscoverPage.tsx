    ...
    <div style={{ backgroundColor: '#f5f5f7' }}>
      ...
      {tags.length > 0 && (
        <div style={{ padding: '16px 16px 4px' }}>
          <h2 style={{ fontSize: 17, fontWeight: 700, color: '#1a1a1a', marginBottom: 8 }}>热门话题</h2>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 4 }}>
            {tags.map((t) => (
              <Link key={t.tag} to={`/search?q=${encodeURIComponent(t.tag)}&type=posts`} style={{ flexShrink: 0, padding: '6px 12px', backgroundColor: '#f5f5f7', borderRadius: 12, fontSize: 13, color: '#1a1a1a', textDecoration: 'none', fontWeight: 500 }}>
                #{t.tag} <span style={{ color: '#999', marginLeft: 2 }}>{t.count}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {circles.length > 0 && (
        <div style={{ padding: '16px 16px 4px' }}>
          <h2 style={{ fontSize: 17, fontWeight: 700, color: '#1a1a1a', marginBottom: 8 }}>热门圈子</h2>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 4 }}>
            {circles.map((c) => {
              ...
              return (
                <Link key={c.id} to={`/circle/${c.id}`} style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#fff', borderRadius: 12, padding: '8px 12px', minWidth: 80, textDecoration: 'none', color: 'inherit' }}>
                  ...
                  <span style={{ fontSize: 11, color: '#999' }}>{members} 人</span>
                </Link>
              )
            })}
          </div>
      ...
      <div style={{ padding: '16px 16px 4px' }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: '#1a1a1a', marginBottom: 8 }}>热门内容</h2>
      </div>
